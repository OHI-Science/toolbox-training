...
1. Make sure you have up-to-date versions of `R` and RStudio and have RStudio configured with Git/GitHub <!--- TODO (Chapter \@ref(datascience))--->
...
Details were covered in Chapter \@ref(toolbox). This figure highlights the files we will focus on in this tutorial: 

![](https://docs.google.com/drawings/d/11QcBxrUNQnzuvJBLcJLMp0C7jFvZ-mz4vIj3OkvlxZ8/pub?h=500)

Remember that the ecosystem structure of any OHI assessment is the same, so as you learn to navigate through and calculate scores in the **toolbox-demo** repository you are also learning how to navigate through and calculate scores in any other OHI assessment repository — yours or anyone else's.

In our **toolbox-demo** repo, heres are a few things to highlight: 
...
![](https://docs.google.com/drawings/d/1ED1Xk2WELXm5y8edbWr_s9kjvm3TpSK80Via05e9Cog/pub?w=960&h=192)
...
Remember from `calculate_scores.r` that after `configure_toolbox.r` runs, the next thing that happens is that `CalculateAll()` runs through the goal models and calculates Status and Trend. Status and Trend calculations are within `R` functions for each goal model in `functions.r`. `functions.r` is in the `conf` folder.
...
![](https://docs.google.com/drawings/d/1OhA7p5-dmrk6dGeP_gWEXFD2pI1ik8KhUYNUSLUVgd4/pub?h=450)
