...
Now that we have prepared and saved our data layer, we'll register it in `layers.csv`. `layers.csv`is a registry that will direct `ohicore` to appropriate data layers, and has information about each data layer - which goal it is used for, filename, column names, etc. For futher detail see Chapter \@ref(prep-data).
...
Let's say we want to tailor the AO goal model by adding a new variable into the equation: poverty.
...
We'll start by preparing, saving, and registering the data layer, and then we'll update the goal model in `functions.r`. 


### Prepare and save a new data layer

We have prepared a short example with AO to prepare a new data layer. 

Open `toolbox-demo/prep/AO/poverty_prep.R` and source it after reading it through. The result will be a new data layer saved to the "layers" folder, and you should see that there is a new file saved in your Git window. 

### Register this new data layer 

Now that we have prepared and saved our data layer, we'll register it in `layers.csv` like we did before. 

This time, since we have added an additional data layer that has not been previously registered, we need to add a new row. 

Open `region2016/layers.csv` in a spreadsheet software (i.e. Microsoft Excel or Open Office). Add a new row near "ao_access", and fill in the following information:
...
First, we'll have to load our data layer with `SelectLayersData()`
...
Next, we'll tailor the goal model itself. 
...
We can check that this is working properly as we go along by running the model line-by-line after loading `status_year=2015; sustainability=1.0`. 

Finally, let's restart R and rerun `calculate_scores.r`.


## Recap of this tutorial 

We have just successfully run through the basic workflow to calculate OHI scores with several variations using our **toolbox-demo** repository. 
