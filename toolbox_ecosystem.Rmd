...
When you first open the repository, in the main folder, you'll encounter these files and folders: 
...
- `prep` is a data preparation folder. As the name suggests, this is where you will store raw data, explore and format those data, and make trial runs of different models. You will spend roughly the first half of your time during an assessment here.
...
Within `prep`, there is a sub-folder for each goal and subgoal, as well as pressures and resilience.

Click on the `CW' folder. Within each goal folder, there are typically a _.Rmd_ script and a sub-folder to contain data. 

The `CW_data_prep.Rmd` document is a script where you will explore your raw data, format the data, try different models, make graphs and maps, as well as document decision makings. We will come back to this script in Chapter \@ref(data prep) to learn how to prepare data for calculations. 
...
`goals.csv` is another spreadsheet file that's best to be opened in Excel. 
...
_We have just gone over the files in the `conf` folder. To recap, `conf` folder contains important configuration files required to calculate OHI scores, including functions.R, pressures and resilience files, etc. Now let's take a step back to the `region2016` folder, and check out what else are there._
...
At the first stage of your assessment, you will spend most of your time in `prep` folder, where you upload, explore, and format your data layers to be saved in the scenario folder `region2016` for the next step of calculations. 
