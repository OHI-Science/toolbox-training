...
- no needs to reintroduce two repos. 
- clone demo repo. follow along to learn different folders and scripts
- prereq. steal from basic workflow. 
- no deleting manual materials. make notes on what's moved over, changed, etc. 

## Overview

In this chapter, we will introduce you to the OHI Toolbox ecosystem. The Toolbox is the engine behind an OHI+ assessment. It allows you to store data, do calculations, as well as record decision-makings and produce reports. Here let's take a look at what files are in there, their purpose, organization, and how they work together. 

The Toolbox mainly consists of two repositories, or two places where files are stored - The Starter Repository and the Full Repository. The Starter Repository, as the name suggests, gets you started with data preparation, while the Full repository contains more scripts and files that help you complete calculations and make reports.

When you work on your OHI+ Planner to begin conducting your assessment, and even before you have defined your regions, we can immediately provide you with a Starter Github repository (_Starter Repo_) to get you familiar with the Github/RStudio workflow, and to help you through the data exploration and region-defining stage.
...
Within each folder, it's up to you how to populate and organize the contents. We recommend that within each folder you save the raw data files, and create a data preparation script (eg. `CW_data_prep.R` or `CW_data_prep.Rmd` for Clean Water) to explore data and document decision making.
...
- a shortened name for the repository (our convention is a **3-letter code**). For example, the Gulf of Guayaquil assessment had `gye` as their repository name.
